...
export function OnboardingRoutePage() {
  ...
  const title = matchedCompany
    ? isKorean
      ? `${matchedCompany.name}에 에이전트 추가하기`
      : `Add another agent to ${matchedCompany.name}`
    : companies.length > 0
      ? isKorean ? "조직 하나 더 만들기" : "Create another organization"
      : isKorean ? "첫 조직 만들기" : "Create your first organization";
  const description = matchedCompany
    ? isKorean
      ? "온보딩을 다시 실행해서 이 조직에 에이전트와 첫 작업을 추가해요."
      : "Run onboarding again to add an agent and a starter task for this organization."
    : companies.length > 0
      ? isKorean
        ? "온보딩을 다시 실행해서 새 조직과 첫 에이전트를 만들어요."
        : "Run onboarding again to create another organization and seed its first agent."
      : isKorean
        ? "조직과 첫 에이전트를 만들면서 시작해요."
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4">
          ...
          {!matchedCompany && cloudInstance && !createStackUrl ? (
            ...
            >
              {matchedCompany
                ? isKorean ? "에이전트 추가" : "Add Agent"
                : isKorean ? "온보딩 시작" : "Start Onboarding"}
            </Button>
          )}
        </div>
      </div>
